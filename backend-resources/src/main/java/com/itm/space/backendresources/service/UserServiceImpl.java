...
public class UserServiceImpl implements UserService {
    ...
    public void deleteUserByUsername(String username) {
        try {
            ...
            keycloakClient.realm(realm).users().delete(userRepresentation.getId());
        } catch (RuntimeException ex) {
            log.error("Exception on \"deleteUserByUsername\": ", ex);
            throw new BackendResourcesException(ex.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public UUID getUserIdByUsername(String username) {
        UserRepresentation userRepresentation;
        try {
            userRepresentation = keycloakClient.realm(realm).users().search(username).get(0);
            return UUID.fromString(userRepresentation.getId());
        } catch (RuntimeException ex) {
            log.error("Exception on \"getUserIdByUsername\": ", ex);
            throw new BackendResourcesException(ex.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
